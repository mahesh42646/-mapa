...
export default function CulturalSpaces() {
  const { user } = useAuth();
  ...
  const filteredSpaces = mockSpaces.filter(space => {
    ...
    return (
      ...
      space.createdBy.toLowerCase().includes(term) ||
      space.createdAt.toLowerCase().includes(term) ||
      space.status.toLowerCase().includes(term)
    );
  });
        ...
        <div className="d-flex justify-content-between align-items-center mb-2">
          ...
          <div style={{ width: 400 }}>
            <input
              ...
              placeholder="Search space by name or agent"
          ...
          <table className="w-100 mb-0" style={{ overflow: 'hidden' }}>
            <thead style={{ background: '#F5F6FA' }}>
              <tr style={{ color: '#6B7280', fontWeight: 600, fontSize: 15 }}>
                <th style={{ border: 'none', padding: '16px 12px' }}>Space Name</th>
                <th style={{ border: 'none', padding: '16px 12px' }}>Created by</th>
                <th style={{ border: 'none', padding: '16px 12px' }}>Status</th>
                <th style={{ border: 'none', padding: '16px 12px' }}>Link</th>
                <th style={{ border: 'none', padding: '16px 12px' }}></th>
              </tr>
            </thead>
            <tbody>
              {filteredSpaces.map((space, idx) => (
                <tr key={space.id} style={{ background: idx % 2 === 1 ? '#FAFAFB' : '#fff', borderRadius: 12 }}>
                  <td className="align-middle" style={{ padding: '14px 12px' }}>
                    <div className="fw-bold" style={{ fontSize: 15 }}>{space.name}</div>
                    <div className="text-muted" style={{ fontSize: 13 }}>Type: {space.type}</div>
                  </td>
                  <td className="align-middle" style={{ padding: '14px 12px', fontSize: 14 }}>
                    <div style={{ color: '#111827' }}>{space.createdBy}</div>
                    <div className="text-muted" style={{ fontSize: 13 }}>{space.createdAt}</div>
                  </td>
                  <td className="align-middle" style={{ padding: '14px 12px' }}>
                    {space.status === 'Ativo' && (
                    ...
                    {space.status === 'Inactive' && (
                      <span className="d-inline-flex align-items-center px-3 py-1 rounded fw-semibold" style={{ fontSize: 14, border: '1.5px solid #B0B3B9', background: '#F3F4F6', color: '#6B7280', gap: 6 }}>
                        <i className="bi-x-circle-fill" style={{ fontSize: 16, color: '#6B7280' }}></i>
                        Inactive
                      </span>
                    )}
